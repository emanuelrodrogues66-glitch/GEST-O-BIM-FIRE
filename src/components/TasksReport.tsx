...
export default function TasksReport() {
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, { nome: string; numero: number | null; tasks: TaskRow[] }>()
    for (const r of late) {
      const key = r.project_id
      if (!map.has(key)) {
        map.set(key, { nome: r.projects?.nome || 'Projeto', numero: r.projects?.numero ?? null, tasks: [] })
      }
  ...
  return (
    <div className="space-y-4">
      ...
      {grouped.map((g) => (
        <div key={g.nome + g.numero} className="bg-white border border-slate-200 rounded-xl p-4">
