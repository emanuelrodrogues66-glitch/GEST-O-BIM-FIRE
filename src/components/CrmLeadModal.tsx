          ...
          <div className="space-y-4">
            ...
            <div className="border border-slate-200 rounded-lg p-3 space-y-2">
              <div className="flex items-baseline gap-2">
                <p className="text-[10px] font-semibold uppercase text-slate-400 flex-1">Pagamento</p>
                {form.rag_cadastrado ? (
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700">
                    ✓ no RAG 3{form.rag_cadastrado_em && ` · ${dataBR(form.rag_cadastrado_em)}`}
                  </span>
                ) : (
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-100 text-amber-800">
                    falta lançar no RAG 3
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <Select
                  ...
                  opcoes={[['', '—'], ...FORMAS_PAGAMENTO.map((f) => [f, f] as [string, string])]}
                  onMudar={(v) => campo({ forma_pagamento: v || null } as Partial<Lead>)}
                  travado={!podeEditar}
                />
                <Campo
                  rotulo="Entrada"
                  valor={form.entrada?.toString() || ''}
                  onSalvar={(v) => campo({ entrada: v ? Number(v) : null } as Partial<Lead>)}
                  travado={!podeEditar}
                />
                <Campo
                  rotulo="Parcelas"
                  valor={form.parcelas?.toString() || ''}
                  onSalvar={(v) => campo({ parcelas: v ? Number(v) : null } as Partial<Lead>)}
                  travado={!podeEditar}
                />
              </div>
              ...
              <label className="flex items-center gap-1.5 text-[11px] text-slate-600">
                <input
                  type="checkbox"
                  disabled={!podeEditar}
                  checked={!!form.rag_cadastrado}
                  onChange={(e) => campo({ rag_cadastrado: e.target.checked } as Partial<Lead>)}
                />
                Cobrança já lançada no RAG 3
              </label>

              <p className="text-[10px] text-slate-400">
                Ao vender, isto vira o valor do contrato e as parcelas do projeto na gestão.
              </p>
            </div>
